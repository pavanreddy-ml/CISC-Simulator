...
//       systemControl.PrintToDebugConsole(String.format("  Sub %d at %H from GPR%d, result is %d", systemControl.registers.memory[EA_X], EA_X, GPR_X, systemControl.registers.GPRS[Utils.GPR_Index]));
    ...
    public void SRC(int GPR_X,int Count_X,int RL_X,int AL_X) {
        systemControl.frame.debugPrint("Running SRC");

        String origin = String.format("%16S", Integer.toBinaryString(systemControl.registers.GPRS[GPR_X])).replace(' ', '0');
        StringBuilder result = new StringBuilder();

        if (systemControl.utils.RL == 0) {
            if (origin.substring(16 - systemControl.utils.Count).contains("1")) {
                systemControl.registers.CC2 = true;
                systemControl.frame.debugPrint("  Underflow!");
            }
        } else {
            System.out.println(origin.substring(0, systemControl.utils.Count));
            if (origin.substring(0, systemControl.utils.Count).contains("1")) {
                systemControl.registers.CC1 = true;
                systemControl.frame.debugPrint("  Overflow!");
            }
        }

        if (systemControl.utils.AL == 1) {
            if (systemControl.utils.RL == 0) {
                systemControl.frame.debugPrint("Shifting right");
                for (int j = 0; j < systemControl.utils.Count; j++) {
                    result.append('0');
                }
                result.append(origin, 0, 16 - systemControl.utils.Count);
            } else {
                systemControl.frame.debugPrint("Shifting left");
                result.append(origin.substring(systemControl.utils.Count));
                for (int j = 0; j < systemControl.utils.Count; j++) {
                    result.append('0');
                }
            }
        } else {
            systemControl.frame.debugPrint("Shift arithmetically");
        ...
        systemControl.registers.GPRS[GPR_X] = Integer.parseInt(result.toString(), 2);

        systemControl.frame.debugPrint(String.format("  Prev binary: %s, result: %s", origin, result));
    }

    public void RRC(int GPR_X,int Count_X,int RL_X,int AL_X) {
        systemControl.frame.debugPrint("Running RRC");

        String origin = String.format("%16S", Integer.toBinaryString(systemControl.registers.GPRS[GPR_X])).replace(' ', '0');
        String result;

        if (systemControl.utils.RL == 0) {
            systemControl.frame.debugPrint("Rotating right");
            result = origin.substring(16 - systemControl.utils.Count) + origin.substring(0, 16 - systemControl.utils.Count);
        } else {
            systemControl.frame.debugPrint("Rotating left");
            result = origin.substring(systemControl.utils.Count) + origin.substring(0, systemControl.utils.Count);
        }

        systemControl.registers.GPRS[GPR_X] = Integer.parseInt(result, 2);
    ...
    public void TRAP(int TrapCode_X) {
        if (TrapCode_X > 15)
        {
            systemControl.frame.debugPrint("Illegal Trap Code. MFR2 set to 1. HALTING");
            systemControl.registers.MFR2 = true;
            return;
        }
